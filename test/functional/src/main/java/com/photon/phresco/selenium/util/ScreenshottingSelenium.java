...
public class ScreenshottingSelenium extends DefaultSelenium {
	 ...
	 * solves the problem with incorrect colors in screenshots taken of
	 * Internet Explorer in 256 color mode.
	 * 
	 * @param filename
	 *            the absolute path to the file to be written, e.g.
	 *            "c:\blah\screenshot.png"
	 */
	/*public void captureScreenshot2(String screenshotFilePath) {
		if (this.BROWSER.equalsIgnoreCase(Constants.BROWSER_IE)) {
			new ScreenshotCapturer.TakeScreenshotExe()
					.captureScreenshot(screenshotFilePath);
		} else {
			new ScreenshotCapturer.SeleniumDefault()
					.captureScreenshot(screenshotFilePath);
		}
	}*/
